...
const usage = `Usage: sdscan [options] FILE

Scan a list of domains for services.
...
query options:
  ...
  -tls-hostname HOSTNAME
    Use the provided HOSTNAME during remote server TLS certificate validation.
    Otherwise, theh DNS server name is used.

  -validate
    DNSSEC validate each query response.


examples:
  $ ./sdscan -server 1.1.1.1 -tcp tranco-1m.txt
`
...
func parseOptions() *Options {
	...
	flag.IntVar(&opts.maxCNAMEs, "max-cnames", 5, "")
